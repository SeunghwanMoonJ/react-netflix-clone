...
function Search() {
  ...
  const [movieData, setMovieData] = useState<ISearchedDataItem[]>();
  ...
  async function getSearched() {
    const response = await fetch(
      `https://api.themoviedb.org/3/search/keyword?api_key=d874fc05ccd756a9cebe2faf5d05ab82&query=${keyword}&page=1`
    );
    const json = await response.json();
    setSearchedData(json);
  }

  const API_OPTION = "search";
  ...
  const onBoxClicked = async (movieId: string) => {
    history.push(`/movie/${API_OPTION}/${movieId}`);
  ...
  //   datas?.results.find((tv) => tv.id === +bigMovieMatch.params.tvId);
          ...
          <AnimatePresence initial={false} onExitComplete={setLeavingFalse}>
            <RowDiv>
              ...
              <Row
                ...
                exit="exit"
              >
                {searchedData?.results.map((movie, index) => (
                  <Box
            ...
            >
              {getClickedMovies ? (
                <>
                  ...
                  <BigBoxTitle
                    style={
                      getClickedMovies.name.length > 25
                        ? { top: -95 }
                        : { top: -60 }
                    }
                  >
                    {getClickedMovies.name}
                  </BigBoxTitle>
                  ...
                  {clickedMoiveData?.last_episode_to_air.air_date ? (
                    ...
                    >
                      ...
                      <BigBoxOverviewButtonSvg
                        onClick={() =>
                          onBigBoxOverviewButtonClicked("searchedData")
                        }
